...
// Import the consolidated ChatService
...
io.on('connection', (socket) => {
  ...
  socket.on('welcome', async (data) => {
    ...
    if (mcpContexts.has(sessionId)) {
      ...
      if (context.memory.identity.is_admin !== isAdmin) {
  ...
  socket.on('loadCustomer', async (data) => {
    ...
    if (!mcpContexts.has(sessionId) || 
        !mcpContexts.get(sessionId).memory.identity || 
        mcpContexts.get(sessionId).memory.identity.is_admin !== isAdmin) {
      const newContext = await createNewMCPContext(sessionId, isAdmin);
      mcpContexts.set(sessionId, newContext);
    }
    ...
    try {
      ...
      context.memory.identity.user_id = customer.resourceName;
      context.memory.identity.persona = "returning_customer";
      
      context.memory.goals = ["provide_service_information", "book_appointment"];
      context.memory.instructions = `Assist ${customer.name} with booking appointments. Personalize the conversation and reference their history if available.`;
  ...
  socket.on('chat', async (data) => {
    try {
      ...
      if (messageContent.toLowerCase().includes('appointment') || messageContent.toLowerCase().includes('book')) {
        if (!context.memory.goals.includes('book_appointment')) {
          context.memory.goals.push('book_appointment');
        }
      }
      
      if (messageContent.toLowerCase().includes('service') || messageContent.toLowerCase().includes('price')) {
        if (!context.memory.goals.includes('provide_service_information')) {
          context.memory.goals.push('provide_service_information');
        }
      }
      ...
      try {
        ...
        const executor = await chatService.getExecutor(sessionId, context, isAdmin);
