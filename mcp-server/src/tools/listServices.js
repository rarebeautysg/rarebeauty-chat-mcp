  ...
  constructor(context, sessionId) {
    ...
    this.schema = { 
      ...
      properties: {
        category: {
          type: 'string',
          description: 'Optional category to filter services (e.g., Lashes, Facial, Threading, Waxing)'
        },
        highlightServices: {
          type: 'array',
          description: 'Service names that should be highlighted or saved in memory',
          items: {
            type: 'string'
          }
        }
  ...
  async _call(args) {
    ...
    try {
      ...
      if (this.context && this.context.memory) {
        ...
        // Check for service removal hints in context
        if (this.context.memory.actionHints && Array.isArray(this.context.memory.actionHints)) {
          const removalHints = this.context.memory.actionHints.filter(
            h => h.type === 'serviceRemoval' && h.serviceName
          );
          
          if (removalHints.length > 0) {
            console.log(`🔍 Found ${removalHints.length} service removal hints in context`);
            
            // Get the current selected services (if any)
            const currentServices = this.context.memory.selected_services_details || [];
            
            if (currentServices.length > 0) {
              console.log(`🔍 Found ${currentServices.length} existing services in context`);
              
              // For each removal hint, try to find matching services to remove
              for (const hint of removalHints) {
                const serviceToRemove = hint.serviceName.toLowerCase();
                console.log(`🔍 Processing removal hint for: "${serviceToRemove}"`);
                
                // Filter out the services that should be removed
                const remainingServices = currentServices.filter(service => {
                  const match = service.name.toLowerCase().includes(serviceToRemove);
                  if (match) {
                    console.log(`🔄 Removing service: ${service.name} (${service.id})`);
                  }
                  return !match;
                });
                
                if (remainingServices.length < currentServices.length) {
                  console.log(`🔄 After removal: ${remainingServices.length} services remain`);
                  
                  // Auto-fill highlightServices with remaining services if not provided
                  if (!args.highlightServices || args.highlightServices.length === 0) {
                    args.highlightServices = remainingServices.map(s => s.name);
                    console.log(`🔄 Auto-filled highlightServices with remaining services:`, args.highlightServices);
                  }
                }
              }
            }
      ...
      // Process the highlighted services if provided
      if (args.highlightServices && args.highlightServices.length > 0) {
        console.log(`🔍 Services highlighted by LLM:`, args.highlightServices);
        
        // Import selectServices functionality directly to avoid circular dependencies
        try {
          // Get all services for matching
          const allServices = await getAllFormattedServices();
          const highlightedServices = [];
          
          // Match each highlighted service name to actual services
          for (const serviceName of args.highlightServices) {
            ...
            if (exactMatch) {
              ...
              console.log(`✅ Exact service match: "${trimmedName}" → ${exactMatch.name} (${exactMatch.id})`);
              continue;
            }
            ...
            if (partialMatches.length > 0) {
              ...
              console.log(`✅ Partial service match: "${trimmedName}" → ${bestMatch.name} (${bestMatch.id})`);
            } else {
              console.log(`❌ No match found for service: "${trimmedName}"`);
            }
          }
          
          // Store in context memory
          if (highlightedServices.length > 0 && this.context && this.context.memory) {
            // Store just the IDs for backward compatibility
            this.context.memory.last_selected_services = highlightedServices.map(service => service.id);
            
            // Store full service details
            this.context.memory.selected_services_details = highlightedServices;
            
            console.log(`🔄 SERVICES AUTOMATICALLY STORED IN MEMORY FROM HIGHLIGHTS: ${highlightedServices.map(s => s.name).join(', ')}`);
          }
        } catch (err) {
          console.error(`❌ Error processing highlighted services:`, err);
        }
      }
